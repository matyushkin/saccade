...
\midrule
Saccade (this work) & \textbf{142\,px / 2.2°} & \textbf{237\,px / 3.7°} & 12.40° & 200 frames \\
...
Features: 123-D (CLAHE 60-D per eye $\times$ 2 + 3 head-pose values).
Lambda: auto-tuned via hat-matrix LOO CV.

\textbf{Result: 12.40° ± 4.82° (mean ± std across 210,658 test frames, all 15 subjects).}

\textbf{Gap analysis — why 12.4° vs 3.92° for L2CS-Net:}

\begin{enumerate}
  \item \textbf{No pretraining.} L2CS-Net uses leave-one-subject-out cross-validation,
    training on $\approx$200k samples from 14 subjects. Our linear model sees only 200
    calibration samples, which is insufficient to span the full $\pm 30^\circ$ gaze range.
  \item \textbf{Feature coarseness.} Our CLAHE + bilinear-resize produces $10\times6 = 60$
    pixels per eye. L2CS-Net operates on $448\times448$ inputs with ResNet-50.
    Sixty pixels cannot represent gaze direction with 3D precision.
  \item \textbf{Evaluation space.} MPIIGaze measures 3-D gaze direction in a normalized
    camera frame — a harder task than predicting 2-D screen coordinates (E12).
    The 3.7° we achieve in screen space is \emph{not} the same metric.
  \item \textbf{Screen-space simplicity.} For one person in one session, the mapping
    eye-patch $\to$ screen position is near-linear over the typical $\pm15^\circ$ gaze range.
    The MPIIGaze task requires accurate cross-subject generalization over a wider range.
\end{enumerate}

\textbf{Conclusion:} Our 237\,px / 3.7° in screen space (E12) is \emph{not directly comparable}
to L2CS-Net's 3.92° in MPIIGaze normalized space.
Achieving $<5^\circ$ on MPIIGaze requires a CNN pretrained on the full dataset
(Options A/C in Section~\ref{sec:evaluation}).
